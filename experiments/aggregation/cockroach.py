...
class Cockroach(Agent):
    ...
    def check_leave(self):
        m, sd = config["settings"]["mleave"], config["settings"]["sdleave"]
        pleave = np.random.normal(m, sd) - (self.neighbors() / 100)
        #u = np.random.uniform(0.1, 1.0)
        if pleave > config["settings"]["uleave"]:
            return True
            print("bro")
        else:
            return False
    ...
    def update_actions(self) -> None:
        m, sd = config["settings"]["mjoin"], config["settings"]["sdjoin"]  # mean and standard deviation
